...
def lambda_handler(event, context):
    ...
    try:
        ...
        logger.info(f"RESPONSE JSON: {json.dumps(response)}")
        if isinstance(response, dict) and "stopReason" in response:
            stop_reason = response["stopReason"]
            content = response["output"]["message"]["content"]
            content_by_type = content[0]
...
def get_content_by_type(content):
    ...
    for content_obj in content:
        for content_type in content_obj:
            content_by_type[content_type].append(content_obj[content_type])
        return content_by_type


def parse_rationale(content_by_type):
    if "text" in content_by_type:
        rationale = content_by_type["text"]
...
def parse_ask_user(content_by_type):
    try:
        if content_by_type["toolUse"]["name"] == ASK_USER:
            ask_user_question = content_by_type["toolUse"]["input"]["question"]
